...
- **SDF 形状**(spec 004 Phase 3)：圆角/描边由 `UI/ShapeSDF` shader + `UIShape` 零纹理画(矢量级清晰、共享材质合批)，translator 发 `cornerRadius`+`stroke` 不再用 round*/ring* 精灵。**出包前把 `UI/ShapeSDF` 加入 Project Settings → Graphics → Always Included Shaders**。渐变沿用 `UIVertexGradient`(顶点色)。
