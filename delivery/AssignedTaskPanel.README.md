# AssignedTaskPanel 鈥?UI delivery package (for YC-Ego)

## Import
1. Import `AssignedTaskPanel.unitypackage` into the YC-Ego project (exported with .meta -> GUID-faithful).
...
## How to bind (read only `AssignedTaskPanel.binding.json` + the prefab; never reference this project)
...
Human-readable element table: `AssignedTaskPanel.binding.md`.
