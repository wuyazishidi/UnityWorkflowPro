# SelectModulePanel 鈥?UI delivery package (for YC-Ego)

## Import
1. Import `SelectModulePanel.unitypackage` into the YC-Ego project (exported with .meta -> GUID-faithful).
...
## How to bind (read only `SelectModulePanel.binding.json` + the prefab; never reference this project)
...
Human-readable element table: `SelectModulePanel.binding.md`.
