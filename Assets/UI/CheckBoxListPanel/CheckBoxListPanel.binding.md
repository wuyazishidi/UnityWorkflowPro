# CheckBoxListPanel — UI 绑定描述

- prefab：`Assets/UI/CheckBoxListPanel/CheckBoxListPanel.prefab`
- 依赖：字体 `MiSans Medium SDF`；公共精灵 `ring12`, `ring6`, `round16`, `round6`；图集 `Assets/UI/CheckBoxListPanel/CheckBoxListPanel.spriteatlas`；needsTmpEssentials = `true`

YC-Ego 据 `key` 绑事件（回退用 `path`），不接触 Figma。

| key | 类型 | 文案 | path | 备注 |
|-----|------|------|------|------|
| `returnBtn` | Button | 返回 | `Container/Container/Return_Btn` |  |
| `taskDescriptionText` | Text | 任务描述： | `Container_Image/Container/TaskDescription_Text` |  |
